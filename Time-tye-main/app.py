...
from tracking.metrics import calculate_metrics, calculate_splits, format_duration, format_pace
...
def get_weekly_stats(user_id: int) -> list[dict]:
    with get_connection() as connection:
        rows = connection.execute(
            """SELECT date(started_at) AS day, COALESCE(SUM(distance_km), 0) AS distance
               FROM runs
               WHERE user_id = ? AND date(started_at) >= date('now', '-6 days')
...
def apply_session_defaults() -> None:
    valid_pages = ["Início", "Correr", "Corredores ao vivo", "Minhas corridas", "Perfil", "Configurações"]
    query_page = st.query_params.get("page")
    default_page = query_page if query_page in valid_pages else "Início"
...
def restore_session() -> None:
    ...
    if open_run:
        ...
        query_page = st.query_params.get("page")
        if not query_page or query_page == "Correr":
            st.session_state.page = "Correr"
            st.query_params["page"] = "Correr"
...
def render_sidebar() -> None:
    ...
    with st.sidebar:
        ...
        selected = st.radio("Navegação", pages, index=pages.index(current_page))
...
def render_dashboard() -> None:
    ...
    if st.session_state.active_run:
        st.markdown(
            """
            <div style="background: rgba(21, 128, 61, 0.12); border: 2px solid #15803d; border-radius: 18px; padding: 1.2rem 1.4rem; margin-bottom: 1.2rem;">
                <div style="display:flex; align-items:center; gap: 0.75rem;">
                    <span style="font-size: 1.6rem;">🏃</span>
                    <div>
                        <h3 style="margin: 0; color: #102a27; font-size: 1.15rem;">Corrida em andamento detectada</h3>
                        <p style="margin: 0.2rem 0 0; color: #166534; font-size: 0.92rem;">Seus dados continuam gravados com segurança. Clique abaixo para voltar à tela da corrida.</p>
                    </div>
                </div>
            </div>
            """,
        ...
        if st.button("RETOMAR CORRIDA EM ANDAMENTO", type="primary", use_container_width=True):
            st.session_state.page = "Correr"
            st.query_params["page"] = "Correr"
            st.rerun()
    else:
    ...
    weekly_data = get_weekly_stats(user["id"])
    if weekly_data:
        st.markdown('<h3 class="section-title">📊 Distância acumulada (últimos 7 dias)</h3>', unsafe_allow_html=True)
        chart_df = pd.DataFrame(weekly_data)
        chart_df["day"] = pd.to_datetime(chart_df["day"]).dt.strftime("%d/%m")
        chart_df = chart_df.set_index("day")
        chart_df.columns = ["Distância (km)"]
        st.bar_chart(chart_df, height=180, color="#15803d")
...
def start_run_form() -> None:
    ...
    st.markdown("O GPS será usado enquanto esta tela estiver aberta. A tela será mantida ativa pelo aplicativo para evitar interrupções.")
    with st.form("start_run_form"):
        visibility_label = st.selectbox("Quem pode ver você correndo?", ["Privado", "Amigos (em breve)", "Público"])
        goal_option = st.selectbox("🎯 Meta de treino (opcional)", ["Livre (sem meta)", "Distância (km)", "Tempo (minutos)"])
        goal_value = None
        if goal_option == "Distância (km)":
            goal_value = st.number_input("Distância alvo (km)", min_value=0.5, max_value=100.0, value=5.0, step=0.5)
        elif goal_option == "Tempo (minutos)":
            goal_value = st.number_input("Tempo alvo (minutos)", min_value=1, max_value=300, value=30, step=5)
        ...
        consent = st.checkbox("Entendo que o navegador pode interromper o GPS se o aplicativo for fechado.", value=False)
        if st.form_submit_button("Iniciar corrida", type="primary", use_container_width=True):
            if not consent:
                st.error("Confirme a ciência das limitações do navegador para continuar.")
                return
...
def render_active_run() -> None:
    ...
    # Auto-refresh timer every 2 seconds for live clock and continuous updates
    st_autorefresh(interval=2000, key=f"active_run_refresh_{active['run_id']}")

    location = get_current_location(key=f"gps_run_{active['run_id']}", interval_ms=3000)
    ...
    if location["available"]:
        st.success("GPS conectado · Tela mantida ativa para gravação contínua" if location.get("accuracy") else "GPS conectado")
    else:
    ...
    goal_opt = active.get("goal_option")
    goal_val = active.get("goal_value")
    if goal_opt == "Distância (km)" and goal_val:
        target = float(goal_val)
        curr = metrics["distance_km"]
        pct = min(1.0, curr / target) if target > 0 else 0.0
        rem = max(0.0, target - curr)
        st.progress(pct, text=f"🎯 Meta: {curr:.2f} / {target:.1f} km ({pct*100:.0f}%) · Faltam {rem:.2f} km")
    elif goal_opt == "Tempo (minutos)" and goal_val:
        target_secs = float(goal_val) * 60
        curr_secs = metrics["duration_seconds"]
        pct = min(1.0, curr_secs / target_secs) if target_secs > 0 else 0.0
        st.progress(pct, text=f"🎯 Meta: {format_duration(curr_secs)} / {format_duration(target_secs)} ({pct*100:.0f}%)")
    ...
    # Live splits expander
    splits = metrics.get("splits", [])
    if splits:
        with st.expander(f"⏱️ Parciais Km a Km ({len(splits)} registros)", expanded=False):
            split_rows = []
            for s in splits:
                label = f"Km {s['km']}" if not s.get("is_partial") else f"Km {s['km']} ({s['distance_km']} km)"
                split_rows.append({"Quilômetro": label, "Tempo": s["formatted_time"], "Ritmo": s["formatted_pace"]})
    ...
    with buttons[0]:
        if not active.get("paused_at"):
            ...
            if st.button("Continuar", type="primary", use_container_width=True):
                ...
                st.rerun()
    with buttons[1]:
        st.caption("GPS automático (Tela Ativa)")
    with buttons[2]:
    ...
    if active.get("confirm_finalize"):
        ...
        with confirm:
            if st.button("Sim, finalizar", type="primary", use_container_width=True):
                ...
                st.query_params["page"] = "Minhas corridas"
                st.rerun()
        with cancel:
...
def render_run_detail(run: dict) -> None:
    ...
    if points:
        gpx_data = generate_gpx(
            points,
            track_name=f"TIME TYE - {run['distance_km']:.2f} km",
            description=f"Corrida gravada em {parse_iso(run['started_at']).strftime('%d/%m/%Y %H:%M')}",
        )
        st.download_button(
            label="📥 Baixar arquivo GPX (compatível com Strava / Garmin)",
            data=gpx_data,
            file_name=f"time_tye_corrida_{run['id']}.gpx",
    ...
    # Per-kilometer splits breakdown
    splits = calculate_splits(points)
    if splits:
        st.markdown('<h3 class="section-title">⏱️ Parciais por Quilômetro (Km a Km)</h3>', unsafe_allow_html=True)
        split_rows = []
        for s in splits:
            label = f"Km {s['km']}" if not s.get("is_partial") else f"Km {s['km']} ({s['distance_km']} km)"
            split_rows.append({
                "Quilômetro": label,
                "Tempo": s["formatted_time"],
                "Ritmo": s["formatted_pace"],
                "Ritmo (min/km)": round(s["pace"], 2),
            })
        splits_df = pd.DataFrame(split_rows)
        col_tbl, col_chart = st.columns([1, 1])
        with col_tbl:
            st.dataframe(splits_df[["Quilômetro", "Tempo", "Ritmo"]], hide_index=True, use_container_width=True)
        with col_chart:
            bar_data = pd.DataFrame({"Ritmo (min/km)": splits_df["Ritmo (min/km)"].values}, index=splits_df["Quilômetro"])
            st.bar_chart(bar_data, height=180, color="#15803d")
